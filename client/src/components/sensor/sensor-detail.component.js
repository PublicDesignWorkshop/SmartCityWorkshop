...
require('./sensor-detail.component.scss');
...
export default class SensorDetail extends React.Component {
    ...
    if (this.chart) {
      ...
      if (temp.length > 0) {
        ...
        let newList = sensor.data.filter((object, index) => {
          if (!found && object.x > props.sensor.timestamp - 15 * 60 * 1000) {
            found = true;
            firstIndex = Math.max(index - 1, 0);
          }
          return object.x > props.sensor.timestamp - 15 * 60 * 1000;
        }).asMutable();

        if (newList.length > 0) {
          newList.unshift({x: props.sensor.timestamp - 15 * 60 * 1000, y: newList[0].y});
        ...
        this.chart.options.timecurrent = props.sensor.timecurrent;
        this.chart.update();
      }
    }
  }
    ...
    if (canvas) {
      ...
      if (temp.length > 0) {
        ...
        this.chart = new Chart(ctx).Scatter(data, {
            ...
            showCustomClickEventCallback: function(timecurrent) {
              props.dispatch({type: "SET_SENSOR_TIMECURRENT", payload: timecurrent});
            }.bind(this),
            timecurrent: props.sensor.timecurrent,
            pointDot: false,
  ...
  render() {
    ...
    return <div ref="wrapper" className="sensor-detail">
